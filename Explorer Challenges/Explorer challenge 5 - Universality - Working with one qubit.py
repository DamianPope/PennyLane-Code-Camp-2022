# -*- coding: utf-8 -*-
"""
Created on Sat Nov 12 17:14:26 2022
...
def get_matrix(params):
    ...
    approximate_U = RZ1 
    approximate_U = np.matmul(RX,approximate_U)
    approximate_U = np.matmul(RZ2,approximate_U)
    approximate_U = np.exp(1j*phi)*approximate_U
        
    return approximate_U
...
def error(U, params):
    ...
    for i in range(2):
        for j in range(2):
            errorSum += (abs(matrix[i,j]-U[i][j]))**2
